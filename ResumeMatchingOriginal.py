...
def evaluate_candidates_comprehensive(job_id: int):
    ...
    Args:
        ...
        api_key (str): OpenAI API key for resume analysis
        
    Returns:
        List[Dict]: List of evaluation results for all candidates
    """
    ...
    try:
        with DatabaseConnection() as db_conn:
            print(f"\nStarting evaluation for Job ID: {job_id}")
            print("=" * 80)
            ...
            print(f"✓ Found {len(shortlisted_profiles)} shortlisted candidates")
            ...
            print("✓ Resume directory ready")

            # Process each candidate
            evaluation_results = []
            total_candidates = len(shortlisted_profiles)
            
            for index, profile in enumerate(shortlisted_profiles, 1):
                candidate_id = profile['candidate_id']
                print(f"\nProcessing Candidate {index}/{total_candidates} (ID: {candidate_id})")
                print("-" * 80)
                ...
                try:
                    ...
                    if not candidates or not candidates[0].get('s3_key_resume'):
                        print(f"⚠ No resume found for candidate ID: {candidate_id}")
                        continue
                    ...
                    if not resume_text:
                        print(f"⚠ Failed to read resume for candidate ID: {candidate_id}")
                        continue
                    print("✓ Resume text extracted")

                    # Skills Analysis
                    print("\nPerforming Skills Analysis...")
                    skills_scores = resume_analyzer.analyze_resume_skills(resume_text, required_skills)
                    
                    if isinstance(skills_scores, dict):
                        for criteria_id, score_data in sorted(skills_scores.items()):
                            print(f"  {score_data['name']}: {score_data['score']}")
                        skills_avg = sum(s['score'] for s in skills_scores.values()) / len(skills_scores) if skills_scores else 0
                        print(f"Skills Average Score: {skills_avg:.2f}")
                    else:
                        print(f"⚠ Error in skills analysis: {skills_scores}")
                        continue
                    ...
                    if isinstance(general_analysis, dict):
                        detailed_scores = general_analysis.get('detailed_scores', {})
                        for criteria_id, score_data in sorted(detailed_scores.items()):
                            print(f"  {score_data['name']}: {score_data['score']}")
                        general_avg = general_analysis.get('overall_score', 0)
                        print(f"General Average Score: {general_avg:.2f}")
                    else:
                        print(f"⚠ Error in general analysis: {general_analysis}")
                        continue
                    ...
                    result = {
                        ...
                        'manager_id': profile['manager_id']  # Include manager_id from shortlisted profile
                    }
                    evaluation_results.append(result)

                    print(f"\nFinal Score: {final_score:.2f}")
                    print("✓ Evaluation complete for this candidate")
                    ...
                    if os.path.exists(local_resume_path):
                        try:
                            ...
                            print("✓ Temporary resume file cleaned up")
                        except Exception as e:
                            print(f"⚠ Warning: Could not delete file {local_resume_path}: {str(e)}")

            # Update scores in database
            if evaluation_results:
                print("\nUpdating scores in database...")
                ResumeScoreUpdater.update_resume_scores(db_conn, evaluation_results)
                print("✓ Database scores updated")
                ...
                for rank, result in enumerate(evaluation_results, 1):
                    ...
                    print(f"Skills Score: {result['skills_score']:.2f}")
                    print(f"General Score: {result['general_score']:.2f}")
                    print("-" * 30)
...
if __name__ == "__main__":
    ...
    # Run the evaluation
    try:
        results = evaluate_candidates_comprehensive(job_id)
